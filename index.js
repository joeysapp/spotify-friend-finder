...
class Animal {
	constructor(uuid, name, cookies, last_seen, coords){
		this.uuid = uuid;
...
$(document).ready(() => {
	...
	$('#location_getter').click((e) => {
		console.log('hi'+navigator.geolocation);
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition((pos) => {
				console.log('ererere');
				console.log(animal_dict[uuid]);
				animal_dict[uuid].coords = pos;
	...
	if (!document.cookie.includes('uuidv4')){
		...
		animal = new Animal(uuid, name, 0, time, 'empty');
...
function confirmAndDisplayAnimals(){
	if (typeof animal_dict['blob'] !== 'undefined'){
		// woo finally got dem
		const animal_dict_copy = Object.assign({}, animal_dict);
		const sorted_ad = _.sortBy(animal_dict_copy, a => a.cookies);
		for (var tmp_animal of sorted_ad){
			// animal = tmp_animal;
			if (tmp_animal.uuid == 'blob'){ continue; }
			console.log(tmp_animal);
			if (tmp_animal.uuid == uuid){
			...
			var div_animal = new Animal(tmp_animal.uuid, tmp_animal.name, tmp_animal.cookies, tmp_animal.last_seen, tmp_animal.cookies);
			$('#animals').append(div_animal.getDiv());
		}
	} else {
